...
function Header()
{
    ...
    $this->Ln(10);
    $this->Cell(210,10,'Reporte Preguntas',0,0,'C');
...
//Llamo a la BD
require ('../../../../modelo/conexion.php');
$busqueda_pregunta=$_SESSION['busqueda_pregunta'];
$consulta = "select id_pregunta, pregunta, estado from tbl_ms_preguntas
where id_pregunta like '%$busqueda_pregunta%' or
        pregunta like '%$busqueda_pregunta%' or
        estado like '%$busqueda_pregunta%' 
order by pregunta";
...
//Imprimimos el header de la tabla
    ...
    $pdf->Cell(95, 10,utf8_decode( 'PREGUNTA'), 1, 0, 'C', 0);
...
while ($row = $resultado->fetch_assoc()) {
    ...
    $pdf->Cell(95, 10,utf8_decode( $row['pregunta']), 1, 0, 'C', 0);
...
$sql_bitacora=$conexion->query("INSERT INTO tbl_ms_bitacora (fecha_bitacora, accion, descripcion,creado_por) value ( '$fecha', 'Reporte Preguntas', 'Genero reporte especifico de preguntas','$sesion_usuario')");
